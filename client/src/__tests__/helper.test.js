...
describe("extractServiceNames", () => {
    it("should extract all service names from nested services", () => {
        const services = {
            nails: [{ name: "Manicure" }, { name: "Pedicure" }],
            hair: [{ name: "Cut" }],
        };
    ...
    it("should return an empty array if no services provided", () => {
...
describe("formatStartTime", () => {
    it("should format a valid slot into time string", () => {
        ...
        const result = formatStartTime(slot);
        expect(result).toMatch(/02:30 PM|14:30/); // handles 12/24h locale differences
    });
...
describe("formatEndTime", () => {
    it("should add duration and return formatted end time", () => {
        const slot = new Date("2025-09-29T14:30:00").toISOString();
        const result = formatEndTime(slot, 45); // +45 minutes
        expect(result).toMatch(/03:15 PM|15:15/);
    });
...
describe("buildNotificationData", () => {
    ...
    it("should build a notification payload with correct fields", () => {
